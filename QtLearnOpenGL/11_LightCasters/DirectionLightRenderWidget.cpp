...
DirectionLightRenderWidget::DirectionLightRenderWidget(QWidget* parent)
...
DirectionLightRenderWidget::~DirectionLightRenderWidget()
{
    qDebug() << __FUNCTION__;
}

void DirectionLightRenderWidget::initializeGL()
{
...
void DirectionLightRenderWidget::resizeGL(int w, int h)
{
...
void DirectionLightRenderWidget::paintGL()
{
    ...
    // 设置光的信息
    m_pShaderProgram->setUniformValue("lightMaterial.direction", m_light.lightdirection);
...
void DirectionLightRenderWidget::keyPressEvent(QKeyEvent* event)
{
    if (m_pCamera)
...
void DirectionLightRenderWidget::mousePressEvent(QMouseEvent* event)
{
    if (m_pCamera)
...
void DirectionLightRenderWidget::mouseMoveEvent(QMouseEvent* event)
{
    if (m_pCamera)
...
void DirectionLightRenderWidget::mouseReleaseEvent(QMouseEvent* event)
{
    if (m_pCamera)
...
void DirectionLightRenderWidget::wheelEvent(QWheelEvent *event)
{
    if (m_pCamera)
...
bool DirectionLightRenderWidget::initShaderProgram(void)
{
    m_pShaderProgram = new QOpenGLShaderProgram(this);

    // 加载顶点着色器
    QString vertexShaderStr(":/11_LightCasters/shader/vertexshader.vsh");
    ...
    // 加载片段着色器
    QString fragmentShaderStr(":/11_LightCasters/shader/fragmentshader.fsh");
...
void DirectionLightRenderWidget::setFillStatus(bool isFill)
{
    m_isFill = isFill;
    this->update();
}

bool DirectionLightRenderWidget::isFill(void)
{
    return m_isFill;
}

// 设置/获取物体的材质
void DirectionLightRenderWidget::setObjectMaterial(const ObjectMaterial& material)
{
    m_objectMaterial = material;
    this->update();
}

DirectionLightRenderWidget::ObjectMaterial DirectionLightRenderWidget::getObjectMaterial(void)
{
    return m_objectMaterial;
}

// 设置/获取光的属性信息
void DirectionLightRenderWidget::setLightInfo(const LightInfo& info)
{
    m_light = info;
    this->update();
}

DirectionLightRenderWidget::LightInfo DirectionLightRenderWidget::getLightInfo(void)
{
    return m_light;
}

void DirectionLightRenderWidget::initObjectMaterial(void)
{
...
void DirectionLightRenderWidget::initLightInfo(void)
{
    m_light.lightdirection = QVector3D(-0.2f, -1.0f, -0.3f);
...
void DirectionLightRenderWidget::initModelData(void)
{
