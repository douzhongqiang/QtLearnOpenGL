...
{
    // 计算点到光源的距离
    float distance = length(lightMaterial.lightPos - M_ObjectPos);
    // 计算光的衰减
    float attenuation = 1.0 / (lightMaterial.constant + lightMaterial.linear * distance + lightMaterial.quadratic * (distance * distance));
