...
CameraRenderWidget::CameraRenderWidget(QWidget* parent)
    :QOpenGLWidget(parent)
{

}

CameraRenderWidget::~CameraRenderWidget()
{
    qDebug() << __FUNCTION__;
}

void CameraRenderWidget::initializeGL()
{
...
void CameraRenderWidget::resizeGL(int w, int h)
{
...
void CameraRenderWidget::paintGL()
{
    ...
    for (int i = 0; i<m_transPos.size(); ++i)
    {
        ...
        float x = qSin(qDegreesToRadians(m_angle)) * r;
        float z = qCos(qDegreesToRadians(m_angle)) * r;
        m_VMat.lookAt(QVector3D(x, 0.0f, z), QVector3D(0.0f, 0.0f, 0.0f), QVector3D(0, 1.0f, 0.0f));
...
bool CameraRenderWidget::initShaderProgram(void)
{
...
void CameraRenderWidget::setFillStatus(bool isFill)
{
    m_isFill = isFill;
    this->update();
}

bool CameraRenderWidget::isFill(void)
{
    return m_isFill;
}

void CameraRenderWidget::setMixNumber(float number)
{
    m_mixNumber = number;
    this->update();
}

float CameraRenderWidget::getMixNumber(void)
{
    return m_mixNumber;
}

void CameraRenderWidget::initModelData(void)
{
    m_points.clear();
...
void CameraRenderWidget::initTimer(void)
{
    m_pTimer = new QTimer(this);
    m_pTimer->setInterval(50);
    QObject::connect(m_pTimer, &QTimer::timeout, this, &CameraRenderWidget::onTimeout);
    m_pTimer->start();
}

void CameraRenderWidget::onTimeout(void)
{
    m_angle += 2.0;
