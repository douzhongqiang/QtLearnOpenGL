...
CubemapsAttrNode::CubemapsAttrNode(QObject* parent)
    :NDNodeBase(parent)
{
    initAttribute();
}

CubemapsAttrNode::~CubemapsAttrNode()
{

}

void CubemapsAttrNode::initAttribute(void)
{
    // 添加光的材质信息
    ...
    QObject::connect(m_pLightPostionAttr, &NDRealAttribute::valueChanged, this, &CubemapsAttrNode::attributeValueChanged);
    ...
    QObject::connect(m_pLightAmbientColorAttr, &NDRealAttribute::valueChanged, this, &CubemapsAttrNode::attributeValueChanged);
    ...
    QObject::connect(m_pLightDiffuseColorAttr, &NDRealAttribute::valueChanged, this, &CubemapsAttrNode::attributeValueChanged);
    ...
    QObject::connect(m_pLightSpecularColorAttr, &NDRealAttribute::valueChanged, this, &CubemapsAttrNode::attributeValueChanged);
    ...
    QObject::connect(m_pCameraPosAttr, &NDRealAttribute::valueChanged, this, &CubemapsAttrNode::attributeValueChanged);
    ...
    QObject::connect(m_pCameraFront, &NDRealAttribute::valueChanged, this, &CubemapsAttrNode::attributeValueChanged);
...
NDAttributeGroup* CubemapsAttrNode::getCullFaceAttrGroup(void)
{
    // 面剔除
    ...
    QObject::connect(m_pCullFaceEnabledAttr, &NDBoolAttribute::valueChanged, this, &CubemapsAttrNode::attributeValueChanged);
    ...
    QObject::connect(m_pCullFaceTypeAttr, &NDString2Attribute::valueChanged, this, &CubemapsAttrNode::attributeValueChanged);
    ...
    QObject::connect(m_pFrontFaceTypeAttr, &NDString2Attribute::valueChanged, this, &CubemapsAttrNode::attributeValueChanged);

    return pCullFaceAttrGroup;
}

void CubemapsAttrNode::setLightInfo(const CubemapsRenderWidget::LightInfo& lightInfo)
{
...
CubemapsRenderWidget::LightInfo CubemapsAttrNode::getLightInfo(void)
{
    CubemapsRenderWidget::LightInfo lightInfo;
...
void CubemapsAttrNode::setCameraPostion(const QVector3D& cameraPos)
{
    m_pCameraPosAttr->setValue(cameraPos);
}

QVector3D CubemapsAttrNode::getCameraPostion()
{
    return m_pCameraPosAttr->getValue().value<QVector3D>();
}

void CubemapsAttrNode::setCameraFront(const QVector3D& cameraFront)
{
    m_pCameraFront->setValue(cameraFront);
}

QVector3D CubemapsAttrNode::getCameraFront(void)
{
    return m_pCameraFront->getValue().value<QVector3D>();
}

// 面剔除相关
void CubemapsAttrNode::setCullFaceEnabled(bool isEnabled)
{
    m_pCullFaceEnabledAttr->setValue(isEnabled);
}

bool CubemapsAttrNode::isCullFaceEnabled(void)
{
    return m_pCullFaceEnabledAttr->getValue().toBool();
}

// 设置要剔除的面
void CubemapsAttrNode::setCullFaceType(CubemapsRenderWidget::CullFaceType type)
{
    m_pCullFaceTypeAttr->setCurrentSelectedIndex((int)type);
}

CubemapsRenderWidget::CullFaceType CubemapsAttrNode::getCullFaceType(void)
{
    return (CubemapsRenderWidget::CullFaceType)m_pCullFaceTypeAttr->getCurrentSelectedIndex();
}

// 设置正面的顺序
void CubemapsAttrNode::setFrontOrderType(CubemapsRenderWidget::FrontFaceOrderType type)
{
    m_pFrontFaceTypeAttr->setCurrentSelectedIndex((int)type);
}

CubemapsRenderWidget::FrontFaceOrderType CubemapsAttrNode::getFrontOrderType(void)
{
    return (CubemapsRenderWidget::FrontFaceOrderType)m_pFrontFaceTypeAttr->getCurrentSelectedIndex();
}

// 设置/获取当前的后期处理效果
void CubemapsAttrNode::setCurrentPostProcessType(CubemapsRenderWidget::PostProcessType type)
{
    m_pPostProcessTypeAttr->setCurrentSelectedIndex((int)type);
}

CubemapsRenderWidget::PostProcessType CubemapsAttrNode::getCurrentPostProcessType(void)
{
    return (CubemapsRenderWidget::PostProcessType)m_pPostProcessTypeAttr->getCurrentSelectedIndex();
}
