...
InvertedImageRenderWidget::InvertedImageRenderWidget(QWidget* parent)
...
InvertedImageRenderWidget::~InvertedImageRenderWidget()
{
    qDebug() << __FUNCTION__;
}

void InvertedImageRenderWidget::initializeGL()
{
...
void InvertedImageRenderWidget::resizeGL(int w, int h)
{
...
void InvertedImageRenderWidget::paintGL()
{
    // 开启深度测试
    glEnable(GL_DEPTH_TEST);
    glDepthFunc(GL_LESS);
...
void InvertedImageRenderWidget::keyPressEvent(QKeyEvent* event)
{
    if (m_pCamera)
...
void InvertedImageRenderWidget::mousePressEvent(QMouseEvent* event)
{
    if (m_pCamera)
...
void InvertedImageRenderWidget::mouseMoveEvent(QMouseEvent* event)
{
    if (m_pCamera)
...
void InvertedImageRenderWidget::mouseReleaseEvent(QMouseEvent* event)
{
    if (m_pCamera)
...
void InvertedImageRenderWidget::wheelEvent(QWheelEvent *event)
{
    if (m_pCamera)
...
bool InvertedImageRenderWidget::initShaderProgram(void)
{
    // 加载顶点着色器
...
bool InvertedImageRenderWidget::initLightShaderProgram(void)
{
    // 加载顶点着色器
...
bool InvertedImageRenderWidget::initGrassShaderProgram(void)
{
    // 加载顶点着色器
    QString vertexShaderStr(":/14_DepthTest/shader/vertexshader.vsh");
    QOpenGLShader* pVertexShader = new QOpenGLShader(QOpenGLShader::Vertex, this);
...
void InvertedImageRenderWidget::setFillStatus(bool isFill)
{
    m_isFill = isFill;
    this->update();
}

bool InvertedImageRenderWidget::isFill(void)
{
    return m_isFill;
}

// 设置/获取光的属性信息
void InvertedImageRenderWidget::setLightInfo(const LightInfo& info)
{
    m_light = info;
    this->update();
}

InvertedImageRenderWidget::LightInfo InvertedImageRenderWidget::getLightInfo(void)
{
    return m_light;
}

void InvertedImageRenderWidget::setCameraPostion(const QVector3D& postion)
{
    m_pCamera->setCameraPostion(postion);
}

QVector3D InvertedImageRenderWidget::getCameraPostion(void)
{
    return m_pCamera->getCameraPostion();
}

void InvertedImageRenderWidget::setCameraFront(const QVector3D& front)
{
    m_pCamera->setCameraFront(front);
}

QVector3D InvertedImageRenderWidget::getCameraFront(void)
{
    return m_pCamera->getCameraFront();
}

// 设置是否显示为深度测试结果
void InvertedImageRenderWidget::setDepthTestVisible(bool isVisible)
{
    m_bShowDepthTest = isVisible;
    this->update();
}

bool InvertedImageRenderWidget::isDepthTestVisible(void)
{
    return m_bShowDepthTest;
}

void InvertedImageRenderWidget::initLightInfo(void)
{
...
void InvertedImageRenderWidget::initLight(QOpenGLFunctions* f)
{
    m_pLightMesh = new COpenGLMesh(f, m_pLightShaderProgram, this);
    initModelData(m_pLightMesh);
}

void InvertedImageRenderWidget::drawLight(void)
{
...
void InvertedImageRenderWidget::initBox(QOpenGLFunctions* f)
{
...
void InvertedImageRenderWidget::initModelData(COpenGLMesh* pMesh)
{
...
void InvertedImageRenderWidget::drawScaledBox(const QVector3D& pos)
{
    QMatrix4x4 mat;
    mat.translate(pos);
    mat.scale(1.015f);
...
void InvertedImageRenderWidget::initModelData2(void)
{
    CAttributePointArray arrays;
...
void InvertedImageRenderWidget::initModelData3(void)
{
...
void InvertedImageRenderWidget::initFloor(QOpenGLFunctions* f)
{
...
void InvertedImageRenderWidget::drawFloor(void)
{
    QMatrix4x4 mat;
...
// 草
void InvertedImageRenderWidget::initGrass(QOpenGLFunctions* f)
{
...
void InvertedImageRenderWidget::initGrassModelData(COpenGLMesh* pMesh)
{
    CAttributePointArray arrays;
...
void InvertedImageRenderWidget::drawGrass(void)
{
...
void InvertedImageRenderWidget::onTimeout(void)
{
    m_angle += 2.0;
